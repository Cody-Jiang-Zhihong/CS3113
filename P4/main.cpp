...
void Initialize() {
    ...
    displayWindow = SDL_CreateWindow("Mech Soul", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_OPENGL);
    ...
    for (int i = 0; i < ENEMY_COUNT; i++) {
        ...
        if (i == 0) {
            ...
            enemyTextureID = LoadTexture("enemies1.png");
        }
        else if (i == 1) {
            ...
            enemyTextureID = LoadTexture("enemies3.png");
        }
        else {
            ...
            state.enemies[i].aiState = IDLE;
            enemyTextureID = LoadTexture("enemies2.png");
        }
...
void Render() {
    ...
    if (state.player->defeatedEnemies == true) {
        DrawText(&program, LoadTexture("font1.png"), "You Kill Them ALL!", 0.5f, -0.25f, glm::vec3(1.0, 0, 0));
        std::cout << "win\n";
        state.player->isActive = false;
    }
    else if (state.player->wasDefeated == true || state.player->position.y < -3.25 ) {
        DrawText(&program, LoadTexture("font1.png"), "You Are DEAD!", 0.5f, -0.25f, glm::vec3(1.0, 0, 0));
        std::cout << "lose\n";
        state.player->isActive = false;
    }
