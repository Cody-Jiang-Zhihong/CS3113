...
enum EntityType { PLATFORM, PLAYER, ENEMY, EFFECT, ITEM};
enum AIType { WALKER, GUARD, YDOC };
enum AIState { WALKING, IDLE, ATTACKING };

enum EFFECType { MELEE, SHOOT };
...
public:
    ...
    int animation_cols = 4; // 4x4 pixel asset
    int animation_rows = 4;
    ...
    void ai_ydoc(Entity* player);

    void activate_effect(Entity* player);
    void effect_melee();
    void effect_shoot();
    ...
    void check_collision_Enemy(Entity* enemies, int EFFECT_COUNT);

    bool const check_collision(Entity* other) const;
    bool check_collision_Talk(Entity* other) const;
    ...
    EFFECType     const get_effect_type()      const { return effect_type; };
    ...
    void const set_effect_type(EFFECType new_effect_type) { effect_type = new_effect_type; };
